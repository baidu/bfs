...
struct WriteOptions {
    ...
    bool sync_when_closed;
    int replica;
    WriteOptions() : write_timeout(-1), sync_timeout(-1), close_timeout(-1),
                     sync_when_closed(true), replica(-1) {}
};
...
struct BFSOptoins {
    const char* username;
    const char* passwd;
    BFSOptoins() : username(NULL), passwd(NULL) {}
};
...
public:
    ...
    static bool OpenFileSystem(const char* nameserver, FS** fs,
                               const BFSOptoins options = BFSOptoins());
    /// Create directory
    ...
    virtual int32_t OpenFile(const char* path, int32_t flags, int32_t mode,
                             File** file, const WriteOptions options = WriteOptions()) = 0;
