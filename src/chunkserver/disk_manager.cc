...
#include "chunkserver/disk_manager.h"
...
int64_t DiskManager::DiskQuota() const {
    return disk_quota_;
}

// TODO: concurrent load
bool DiskManager::LoadStorage() {
    bool ret = true;
    for (size_t i = 0; i < disks_.size(); ++i) {
        ret = ret && disks_[i]->LoadStorage(std::bind(&DiskManager::AddBlock,
...
int64_t DiskManager::NameSpaceVersion() {
    int64_t version = -1;
    for (size_t i = 0; i < disks_.size(); ++i) {
...
bool DiskManager::SetNameSpaceVersion(int64_t version) {
    for (size_t i = 0; i < disks_.size(); ++i) {
...
// TODO: need test
int64_t DiskManager::ListBlocks(std::vector<BlockMeta>* blocks, int64_t offset, uint32_t num) {
...
Block* DiskManager::CreateBlock(int64_t block_id, StatusCode* status) {
    BlockMeta meta;
...
bool DiskManager::CloseBlock(Block* block) {
    return block->Close();
}

StatusCode DiskManager::RemoveBlock(int64_t block_id) {
    Block* block = FindBlock(block_id);
    if (!block) {
        LOG(INFO, "Try to remove block that does not exist: #%ld ", block_id);
        return kNsNotFound;
    } else {
        MutexLock lock(&mu_, "DiskManager::RemoveBlock erase", 1000);
        block_map_.erase(block_id);
        block->DecRef();
...
bool DiskManager::CleanUp(int64_t namespace_version) {
    for (size_t i = 0; i < disks_.size(); ++i) {
...
bool DiskManager::AddBlock(int64_t block_id, Block* block) {
    MutexLock lock(&mu_);
    return block_map_.insert(std::make_pair(block_id, block)).second;
}

Block* DiskManager::FindBlock(int64_t block_id) {
    g_find_ops.Inc();
...
bool DiskManager::EraseBlock(int64_t block_id) {
    MutexLock lock(&mu_);
    return block_map_.erase(block_id) == 1;
}

Disk* DiskManager::PickDisk(int64_t block_id) {
