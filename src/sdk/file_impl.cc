...
int32_t FileImpl::Close() {
    ...
    if (need_report_finish) {
        ...
        bool rpc_success = fs_->nameserver_client_->SendRequest(&NameServer_Stub::FinishBlock,
                                                   &request, &response, 15, 1);
        if (!rpc_success || response.status() != kOK)  {
