...
bool CreateTree(NameSpace* ns) {
    ...
    int ret = ns->CreateFile("/file1", 0, 0, -1, &block);
    ret |= ns->CreateFile("/file2", 0, 0, -1, &block);
    ret |= ns->CreateFile("/dir1/subdir1/file3", 0, 0, -1, &block);
    ret |= ns->CreateFile("/dir1/subdir1/file4", 0, 0, -1, &block);
    ret |= ns->CreateFile("/dir1/subdir2/file5", 0, 0, -1, &block);
    ret |= ns->CreateFile("/xdir", 0, 01755, -1, &block);
    return ret == 0;
}
...
TEST_F(NameSpaceTest, CreateFile) {
    ...
    NameSpace ns;
    std::vector<int64_t> block;
    ASSERT_EQ(0, ns.CreateFile("/file1", 0, 0, -1, &block));
    ASSERT_NE(0, ns.CreateFile("/file1", 0, 0, -1, &block));
    ASSERT_EQ(0, ns.CreateFile("/file2", 0, 0, 0, &block));
    ASSERT_EQ(0, ns.CreateFile("/file3", 0, 0, 2, &block));
    ASSERT_EQ(0, ns.CreateFile("/dir1/subdir1/file1", 0, 0, -1, &block));
    ASSERT_EQ(0, ns.CreateFile("/dir1/subdir1/file1", O_TRUNC, 0, -1, &block));
    ASSERT_EQ(0, ns.CreateFile("/dir1/subdir2/file1", 0, 0, -1, &block));
    ASSERT_EQ(0, ns.CreateFile("/dir1/subdir2/file2", 0, -1, -1, &block));
    ASSERT_EQ(0, ns.CreateFile("/dir1/subdir2/file3", 0, 01755, -1, &block));
}
...
TEST_F(NameSpaceTest, Rename) {
    ...
    /// Deep rename
    std::vector<int64_t> block;
    ASSERT_EQ(0, ns.CreateFile("/tera/meta/0/00000001.dbtmp", 0, 0, -1, &block));
...
TEST_F(NameSpaceTest, DeleteDirectory2) {
    ...
    NameSpace ns;
    std::vector<int64_t> block;
    ns.CreateFile("/tera", 0, 01755, -1, &block);
    ns.CreateFile("/file1", 0, 0, -1, &block);
    ns.CreateFile("/tera/file2", 0, 0, -1, &block);
    std::vector<FileInfo> files_removed;
